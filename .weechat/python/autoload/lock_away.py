...
#
# Set away status if lock is running
#
# History:
#
...
SCRIPT_NAME = "lock_away"
SCRIPT_AUTHOR = "Peter A. Shevtsov <pshevtsov@severusweb.ru>"
SCRIPT_VERSION = "0.1"
SCRIPT_LICENSE = "GPL3"
SCRIPT_DESC = "Set away status if lock is running"

SCRIPT_COMMAND = "lock_away"
...
ScreenSaver = screensaver_interface()

TIMER = None

settings = {
        'away_message': 'Away.',
        'interval': '60',  # How often to check for inactivity (in seconds)
        'away': '0'
}
...
def lock_away_cb(data, buffer, args):
    """Callback for /lock_away command"""
    response = {
            'msg': lambda status:
    ...
    if args:
        ...
        if words[0] in response:
            ...
            weechat.prnt('', "lock_away error: %s not a recognized command. "
                       "Try /help lock_away" % words[0])
    weechat.prnt('', "lock_away: away message: \"%s\"" %
